...
// region Node
func newnode(valueptr *Item) *node {
	return &node{
...
func (l *linkedlist) Insert(v *Item) (bool, error) {
	n := newnode(v)
	var left, right *node
	for {
...
func (l *linkedlist) Upsert(v *Item) (bool, error) {
	n := newnode(v)
...
func (l *linkedlist) Delete(v *Item) (bool, error) {
	var right *node
...
func (l *linkedlist) Contains(v *Item) bool {
	key := getid(v)
	_, right := l.search(key)
	if right == l.tailnode() || right.key != key {
		return false
